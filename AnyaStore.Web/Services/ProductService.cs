...
namespace AnyaStore.Web.Services
{
    public class ProductService : IProductService
    {
        ...
        public ProductService(IHttpClientFactory clientFactory, IBaseService baseService, IConfiguration configuration)
        {
        ...
        public async Task<T> GetByCategoryAsync<T>(int id)
        {
            return await _baseService.SendAsync<T>(new RequestDTO()
            {
                ApiType = SD.ApiType.GET,
                Url = $"{productUrl}/api/ProductAPI/GetByCategory/{id}",
            });
        }

        public async Task<T> UpdateAsync<T>(ProductDTO dto)
        {
            return await _baseService.SendAsync<T>(new RequestDTO()
            {
